...
const ingredient_match = (userId) => computed(() => {
  ...
  const commonRecipeIds = userAvailableIngredients.flatMap(ingredient => {
    ...
    return user_recipe_ingredient.value.filter(recipeIngredient => recipeIngredient.ingredient_id === ingredient.ingredient_id)
  ...
  //보유식자재 기준 추천
  const category_3 = (userId)=> computed(() => {
    const recipeIds = ingredient_match(userId).value.map(item => item.id);
  ...
  //console.log("ingredientIds",category_3(1));
